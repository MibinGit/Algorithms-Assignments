...
public class Benchmark<T> {
    ...
    private long doRun(T t, boolean warmup) {
        ...
        fRun.accept(t1);
        long stopTimer = System.nanoTime() - startTimer;
    ...
    public static Integer[] doubling(int n, String randType)
    {
        ...
        switch (randType)
        {
            ...
            case "partially-ordered":
                for (int i = 0; i < n; i++)
                {
                    array[i] = i + (int)Math.pow(-1, random.nextInt(10)) * random.nextInt();
                }
                break;
    ...
    {
        ArrayList<String> randType = new ArrayList();
        randType.add("random");
        randType.add("ordered");
        randType.add("partially-ordered");
        randType.add("reverse-ordered");
        Random random = new Random();
        int m = 100; // This is the number of repetitions: sufficient to give a good mean value of timing
        for(String s : randType)
        {
            ...
            for(int k = 0; k < 5; k++)
            {
                Integer[] array = new Integer[n];
                array = doubling(n, s);
                benchmarkSort(array, "Current Random Type Is:" + s + " " + "Insertion Sort: " + n, new InsertionSort<>(), m);
                n = n * 2;
            }
        }
    }
